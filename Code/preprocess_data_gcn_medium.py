...
with open('/wrk/users/sjsuomel/gcn/gcn/data/ind.medium.y', 'wb') as handle:
    pickle.dump(train_labels, handle, protocol=4)

test_labels = label_matrix[test_index,:]
with open('/wrk/users/sjsuomel/gcn/gcn/data/ind.medium.ty', 'wb') as handle:
    pickle.dump(test_labels, handle, protocol=4)

label_matrix_no_test = np.delete(label_matrix, test_index, axis=0) # remove test index
with open('/wrk/users/sjsuomel/gcn/gcn/data/ind.medium.ally', 'wb') as handle:
    pickle.dump(label_matrix_no_test, handle, protocol=4)

with open('/wrk/users/sjsuomel/gcn/gcn/data/ind.medium.test.index', 'w') as f:
...
with open('/wrk/users/sjsuomel/gcn/gcn/data/ind.medium.graph', 'wb') as handle:
